(function () {
  ...
  function renderShowcase() {
    ...
    stage.innerHTML = rows.map(function (item, index) {
      ...
      return '<div role="button" tabindex="0" type="button" class="ffx-showcase-slide' + (active ? " active" : "") + '" data-ffx-slide="' + index + '" data-nav="' + esc(item.nav) + '" data-key="' + esc(item.key || "") + '" aria-hidden="' + (active ? "false" : "true") + '" tabindex="' + (active ? "0" : "-1") + '">' +
        '<img src="' + esc(item.thumb) + '" alt="' + esc(item.title) + '"><span class="ffx-showcase-copy"><span>0' + (index + 1) + ' · TRENDING</span><h3>' + esc(item.title) + '</h3><p>' + esc(item.desc) + '</p><b>' + esc(item.meta) + '</b></span></div>';
    }).join("");
    progress.innerHTML = rows.map(function (_, index) { return '<div role="button" tabindex="0" type="button" class="' + (index === state.showcase ? "active" : "") + '" data-ffx-show="' + index + '" aria-label="Show template ' + (index + 1) + '"></div>'; }).join("");
  }
  ...
  function renderSessions() {
    ...
    rail.innerHTML = '<div role="button" tabindex="0" type="button" class="ffx-session-card new" data-ffx-new><span>＋</span><b>New session</b></div>' + rows.map(function (session, index) {
      ...
      return '<div role="button" tabindex="0" type="button" class="ffx-session-card" data-ffx-session="' + index + '">' + art + '<div><b>' + esc(sessionTitle(session)) + '</b><small>' + esc(mode.toUpperCase()) + ' · ' + Number(session.count || 0) + ' results</small></div></div>';
    }).join("");
  ...
  function renderPulse() {
    ...
    var hostLabel = isPremiere() ? "Premiere" : "Premiere Pro";
  ...
  function renderTools() {
    ...
    grid.innerHTML = tools.map(function (tool) {
      return '<div role="button" tabindex="0" type="button" class="ffx-tool-card' + (tool.featured ? " featured" : "") + '" data-ffx-tool="' + esc(tool.mode || tool.action) + '" style="--tool-color:' + tool.color + '"><span class="ffx-tool-icon">' + esc(tool.icon) + '</span><strong>' + esc(tool.name) + '</strong><small>' + esc(tool.desc) + '</small></div>';
    }).join("");
  ...
  function ensureToolsModal() {
    ...
    layer.hidden = true;
    layer.innerHTML = '<section class="ffx-tools-dialog" role="dialog" aria-modal="true" aria-labelledby="ffxToolsDialogTitle"><header><div><span>TOOL LIBRARY</span><h2 id="ffxToolsDialogTitle">All FrameFlow tools</h2></div><div role="button" tabindex="0" type="button" data-ffx-close aria-label="Close">×</div></header><input id="ffxToolSearch" type="search" placeholder="Search tools"><div class="ffx-tools-list" id="ffxToolsList"></div></section>';
  ...
  function renderAllTools(query) {
    ...
    host.innerHTML = rows.map(function (tool) {
      return '<div role="button" tabindex="0" type="button" class="ffx-tool-row" data-ffx-all="' + esc(tool.mode || tool.action) + '" style="--tool-color:' + tool.color + '"><span>' + esc(tool.icon) + '</span><div><b>' + esc(tool.name) + '</b><small>' + esc(tool.desc) + '</small></div><em>OPEN ↗</em></div>';
    }).join("");
  }
  function openTools() {
  ...
  function renderHost() {
    ...
    host.innerHTML = (isPremiere() ? "PR · Active sequence" : "Premiere · Active composition") + " <b>" + stateLabel + "</b>";
