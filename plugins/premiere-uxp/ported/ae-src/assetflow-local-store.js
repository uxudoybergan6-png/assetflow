/**
 * AssetFlow storage — browser (IndexedDB) yoki Premiere CEP (disk, ikkala panel uchun umumiy).
 */
const AssetFlowStore = (() => {
  ...
  function saveMeta(items) {
    ...
    if (useDisk) {
      ...
      // Ilgari to'g'ridan `meta.json` ga yozilardi: Premiere yozuv o'rtasida yopilsa
  ...
  async function exportBlobToPath(blobId, fileName) {
    await openDb();
    if (!useDisk) throw new Error("File export is only available in the Premiere panel");
