...
export async function processPreviewTranscode(id: string): Promise<void> {
  ...
  try {
    ...
    // 🔴 P1 (owner 2026-07-14): STOCK preview endi SUV BELGISIZ — TOZA lekin ATAYLAB PAST-RES
    // (DRM = ruxsat/rezolyutsiya). optimizePreviewForStreaming 720p'ga tushiradi (kapl. bitrate).
    // To'liq sifat asl FAQAT `pack` kalitida (private/gated) qoladi. Stock va template previewi
    // endi bir xil quvur — ikkovi ham 720p toza.
