...
// Ommaviy ("ko'rsatish") kalitlar allow-list'i — YAGONA manba (Worker CDN-proksi
// ham AYNAN shuni ishlatadi). public-vs-signed qarori uchun (getPublicOrSignedUrl).
...
// Ommaviy allow-list'ni qayta eksport qilamiz (tashqi importer'lar uchun; asl
// ta'rif ./public-keys.ts da — Worker bilan bir manba). Bucket YOPIQ qoladi
// (GCP org-policy per-obyekt public'ni taqiqlaydi), ko'rsatish assetlari
// cdn.getframeflow.app Worker-proksi orqali beriladi — obyekt ACL'i ISHLATILMAYDI.
export { isPublicReadKey };
...
export async function getPublicOrSignedUrl(
  ...
  // 🔴 CDN yoqilgan bo'lsa ham FAQAT ommaviy allow-list kalitlari CDN URL oladi
  // (cdn.getframeflow.app Worker-proksi ularni beradi). Private obyektlar (gen
  // asl fayli, gen-refs/audio — tashqi provayder yuklab oladigan manbalar) uchun
  // signed URL qaytadi: bucket YOPIQ, Worker bunday kalitga 403 beradi, lekin
  // imzolangan storage.googleapis.com URL bilan provayder/mijoz kira oladi.
