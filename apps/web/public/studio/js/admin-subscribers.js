...
VIEWS["subscriber-detail"] = function (id) {
  ...
  return `<div class="col gap-16">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px" class="sub-detail-grid">
      ...
      <div class="card card-pad col gap-12">
        <span class="label">So\u2018nggi harakatlar (demo)</span>
        <div class="timeline">
          <div class="tl-item"><div class="tl-dot" style="background:var(--green)">${ic("download")}</div><div class="tl-title">Shablon yuklab oldi</div><div class="tl-meta">Clean Lower Thirds Bundle · ${s.lastSeen}</div></div>
          <div class="tl-item"><div class="tl-dot" style="background:var(--violet)">${ic("layers")}</div><div class="tl-title">AE kompozitsiyaga import</div><div class="tl-meta">Browse paneli · 1080p</div></div>
          <div class="tl-item"><div class="tl-dot" style="background:var(--blue)">${ic("plugin")}</div><div class="tl-title">Panel ochildi</div><div class="tl-meta">After Effects ${s.ae}</div></div>
        </div>
      </div>
    </div>
  </div>`;
