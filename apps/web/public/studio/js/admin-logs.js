...
async function refreshLogs(showToast) {
  ...
  try {
    ...
    serverOk = h.ok;
    if (status) status.textContent = serverOk ? "localhost:4000 ulangan" : "server javob bermadi";
  } catch {
...
function clearLogs() {
  openModal(`
    <div class="modal-head"><div class="modal-ico" style="background:var(--red-dim);color:var(--red)">${ic("trash")}</div>
      <div><h3>Loglarni tozalash</h3><p>Brauzer va serverdagi demo loglar o‘chadi.</p></div></div>
...
async function doClearLogs() {
  AssetFlowLog.clear();
  try {
    await fetch("http://localhost:4000/api/logs", { method: "DELETE" });
  } catch {
    /* */
  }
  closeModal();
